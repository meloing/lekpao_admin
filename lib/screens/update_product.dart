...
class UpdateProductScreenState extends State<UpdateProductScreen> {
  ...
  Future updateProduct() async{
    ...
    if(response){
      response = await ProductRequests().addProductInJson(
          product["productId"], int.parse(price), 5);
    }
  ...
  void modal(bool value){
    showDialog(
        ...
        builder: (context) => AlertDialog(
            ...
            content: Text(
                value ? "Modification effectué avec succès" : "Erreur lors de l'ajout"
            ),
            actions: [
              TextButton(
                  onPressed: (){
  ...
  void load(){
    setState(() {
      countrys = product['country'];
      fileTypeValue = product['type'];
      topicLevels = product['levels'];
      dateController.text = product['date'];
      fileController.text = product['file'];
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        appBar: AppBar(
            elevation: 0,
            title: const Text("METTRE A JOUR UNE ANNALE")
        ),
